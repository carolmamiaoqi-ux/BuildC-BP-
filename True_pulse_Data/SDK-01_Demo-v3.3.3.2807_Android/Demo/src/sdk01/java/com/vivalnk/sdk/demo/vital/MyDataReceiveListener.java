...
public class MyDataReceiveListener implements DataReceiveListener {
    ...
    public void onReceiveData(Device device, Map<String, Object> data) {
        ...
        //TODO 文档中写清楚: 提供 handler/无handler接口
        //若有handler，则按照handler post
        //否则按照同步Post的方式给到上层app
        subject.onNext(new Runnable() {
            @Override
            public void run() {

                if (DeviceModel.isECGDevice(device.getModel()) || DeviceModel.isHealthDevice(device.getModel())) {
                    //handle sample data in io thread
                    handleSampleData(device, data);
                    return;
                }

                DeviceManager.VitalSampleData liveData = new DeviceManager.VitalSampleData();
    ...
    public void onFrameDataStatusChange(Device device, long remainderFrameData) {
        Log.d(TAG, device.getName() + " remainder frame data num = " + remainderFrameData);
    }
